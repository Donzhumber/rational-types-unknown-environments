...
$\ProbI{i}(\cdot\mid a_t^{i\ast},X_t)$ depends on $X_t$ only through $\mu_t$
...
operate on a frozen deterministic policy path $(\chi_s)_{s \ge 0}$ evaluated
...
$S(s\mid\theta_K):=\prod_{r=1}^{s}p_{\mathrm{Cont},r\mid\theta_K}$ with
$S(0\mid\theta_K)=1$, expected duration admits the survival representation
\begin{equation}
\label{eq:duracion-identidad-prop}
\mathbb{E}[\tau\mid\theta_K]=\sum_{s=0}^{\infty}S(s\mid\theta_K),
\end{equation}
so only factors with $s\ge t$ depend on $\gamma_t^\ast$.
...
=-\kappa_h(\theta_K,t)\,\Delta t\sum_{s=t}^{\infty}S(s\mid\theta_K).
\end{equation}

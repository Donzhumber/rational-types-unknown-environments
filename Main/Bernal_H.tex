% Econometrica final manuscript (ecta_template.tex / ecta_sample.tex)
% English version translated from Identification_of_rational_types_esp.tex
% Econometrica format: econsocart class, Times (ptm), 10.5 pt / 12 pt.
% The final option follows the prepublication format described in the template.
...
\newcommand{\panelgraphic}[2]{%
  \parbox[t][#1][c]{\linewidth}{\centering\includegraphics[width=\linewidth,height=#1,keepaspectratio]{#2}\par}%
}
...
\graphicspath{{./figuras_esp_ordenadas/}}
...
\title{Identifying Rational Types in Unknown Environments}
\runtitle{Identifying Rational Types}
...
\begin{abstract}
This paper analyzes the design of an indirect dynamic mechanism to dismantle criminal organizations under incomplete information, with an application to kidnapping for ransom in Colombia. The empirical framework combines a VEC model that identifies common stochastic trends among extortionate kidnapping, non-extortionate kidnapping, and the commercial active interest rate with microeconometric estimates from multinomial logit and competing-risks models. These results provide the mechanism's prior information by anchoring costs, hazards, initial beliefs, and heterogeneity across criminal types. The central theoretical contribution is the Identification Theorem for Rational Types in Unknown Environments, which converts recurrent separation among observable signals into posterior identification of the true type. The result shows that, under consistent Bayesian updating, the state's beliefs converge almost surely to the true type of the captor. The paper also demonstrates conditional implementability of the mechanism in perfect Bayesian equilibrium, guaranteeing incentive compatibility and participation. Dynamic calibration validates the viability of the mechanism and shows how state instruments accelerate learning and discipline optimal policy.
\end{abstract}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\widefigheight}{Fig_1a.png}
  \caption{Global distribution of kidnapping cases}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\widefigheight}{Fig_1b.png}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_2a.png}
  \caption{Temporal evolution of kidnappings}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_2b.png}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\mapfigheight}{Fig_3a.png}
  \caption{Geographical distribution of GAULA units}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\mapfigheight}{Fig_3b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\scriptsize \textit{Note:} The figure presents the total number of kidnappings per presidential term in Colombia from 1962 to 2025. Each bar corresponds to a presidential administration; the final period corresponds to partial data from the Gustavo Petro administration. Shading distinguishes phases of the armed conflict, and annotations highlight developments related to armed groups, legal reforms, and security policies. The total reported in this period amounts to 61,341 cases. \textit{Source:} Author's elaboration based on \citet{cnmh_datos} and \citet{pinto2004}.
\end{minipage}
\end{figure}
...
\sum_{j=1}^{35}\Gamma_{1,j}\Delta Y_{t-j} + \varepsilon_{1,t}, \label{eq:vec1}\\
\Delta K^{NE}_t &= 0{,}36\,\tilde{\varepsilon}_{t-1} +
\sum_{j=1}^{35}\Gamma_{2,j}\Delta Y_{t-j} + \varepsilon_{2,t}, \label{eq:vec2}\\
\Delta r_t &= -0{,}06\,\tilde{\varepsilon}_{t-1} +
\sum_{j=1}^{35}\Gamma_{3,j}\Delta Y_{t-j} + \varepsilon_{3,t}. \label{eq:vec3}
\end{align}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_5a.png}
  \caption{Gonzalo--Granger common factors}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_5b.png}
...
&& \hat{\sigma}_\zeta \approx 3{,}51\;(p<0{,}001) \\[6pt]
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_6a.png}
  \caption{Trends estimated by LLT and VEC--GG}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_6b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) compares the trends estimated by LLT and VEC--GG, July 1954--December 2025 ($T=858$). Panel (b) shows the difference $d_t$ over the same period. \textit{Source:} Author's calculations based on \citet{cnmh_datos} and the National Police of Colombia.
\end{minipage}
\end{figure}
...
\subsection{Microstructure and competing hazards in Colombia}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_7a.png}
  \caption{State Rescue by captivity duration}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_7b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) reports the probability of state Rescue by captivity duration category. Panel (b) shows the probability of Escape or release without payment by victim profile. \textit{Source:} Author's calculations based on \citet{cnmh_datos}, Stata~19.
\end{minipage}
\end{figure}

The geographic dimension and the identity of the perpetrator group provide two additional sources of structural heterogeneity with direct implications for the mechanism design model. In the territorial dimension, the Rescue probability decreases from 0.465 in metropolitan areas and 0.447 in the Andean region to 0.414 in the Pacific, 0.424 in the East/Jungle, and 0.343 in the Caribbean, a gradient that reflects differences in institutional density, logistical accessibility, and the permanent presence of the GAULA in the territory. In the organizational dimension, the probability of Death in captivity follows an ordering determined by the logistical retention capacity of each organization: 0.002 for the FARC, 0.007 for the ELN, 0.026 for the paramilitaries, and 0.071 for common delinquency. This gradient confirms that the risk of execution is inversely proportional to such capacity; actors with lesser infrastructure face a shorter time horizon before the failure of the negotiation translates into violence. Figure~\ref{fig:mnl-b} presents both patterns.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_8a.png}
  \caption{State Rescue by geographic zone}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_8b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) reports the probability of state Rescue by geographic zone. Panel (b) shows the probability of Death in captivity by responsible group. \textit{Source:} Author's calculations based on \citet{cnmh_datos}, Stata~19.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_9a.png}
  \caption{Rescue by GAULA intervention and duration}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_9b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) reports the probability of Rescue by GAULA intervention and duration category. Panel (b) presents the survival curves of captivity duration by perpetrator group. \textit{Source:} Author's calculations based on \citet{cnmh_datos}, Stata~19.
\end{minipage}
\end{figure}

The cumulative Rescue dynamics confirm that state effectiveness varies with the organizational structure of the perpetrator. Toward day 100, the cumulative Rescue probability reaches 0.33 for paramilitaries, 0.29 for the ELN, and 0.27 for the FARC: the greater territorial exposure and more urban operational positioning of paramilitary groups increase their vulnerability to state intervention in the long run. The mortality profile exhibits an ordering inversely proportional to logistical capacity: the probability of Death is approximately forty times higher in common delinquency (0.079) than in the FARC (0.002), reflecting that actors with less retention infrastructure resort more frequently to violence when negotiations fail. Figure~\ref{fig:cox-b} presents these patterns.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_10a.png}
  \caption{Cumulative state Rescue probability}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\subfigheight}{Fig_10b.png}
...
\vspace{3pt}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) shows the cumulative state Rescue probability by armed group, 1964--2025. Panel (b) reports the probability of Death in captivity by perpetrator group. \textit{Source:} Author's calculations based on \citet{cnmh_datos}, Stata~19.
\end{minipage}
\end{figure}
...
This section formalizes the stochastic architecture that transforms strategic plans into observable realizations. The MDG process transforms plans $a_t^{i\ast}$ into executed actions $\tilde{a}_t^i$ via an implementation law concentrated on $a_t^{i\ast}$ but with full support over $\mathcal{A}^i$, in the spirit of the trembling hand of \citet{Selten1975Perfectness} and \citet{KrepsWilson1982}. This full-support requirement also connects directly with the consistency discipline of \citet{FudenbergTirole1991PBE}: since every branch of the dynamic game is reached with strictly positive probability under the implementation kernel, beliefs can be updated by Bayes' rule at reached information sets and can be selected off the equilibrium path in a way that remains compatible with sequential rationality. Thus, the probabilistic technology is not merely a numerical perturbation; it is the device that makes each branch probabilistically reachable and preserves the Perfect Bayesian consistency of the equilibrium system. The executed profile $\tilde{A}_t=(\tilde{a}_t^K,\tilde{a}_t^S,\tilde{a}_t^F)$ is the observable variable that enters the public record; the strategic intention $a_t^{i\ast}$ remains latent. Given the plan $a_t^{i\ast}\in\mathcal{A}^i$ and the state vector $X_t$ summarizing the system, the execution is determined by
\[
\tilde{a}_t^i\sim\ProbI{i}\bigl(\cdot\mid a_t^{i\ast},X_t\bigr),
\]
where $\ProbI{i}$ satisfies two regularity conditions: \emph{dominance and error support}, whereby the distribution concentrates most of its mass on $a_t^{i\ast}$ while assigning strictly positive probability to alternative actions treated as implementation frictions; and \emph{institutional convergence}, whereby the implementation noise vanishes as the system accumulates information, $\lim_{t\to\infty}\Prob(\tilde{a}_t^i=a_t^{i\ast}\mid\mathcal{F}_t^{\mathrm{pub}})=1$, a condition satisfied when $H(\mu_t)\to 0$.

The logit distribution implementing $\ProbI{i}$ is the \emph{unique} law that simultaneously satisfies full support, dominance, and the maximum entropy of \citet{Shannon1948} subject to those constraints; by the characterization theorem of \citet{Jaynes1957Information}, the unique maximizer is precisely the logit. For each agent $i\in\{K,S,F\}$ and action $a\in\mathcal{A}^i$,
\begin{equation}
\ProbI{i}\bigl(\tilde{a}_t^i=a\mid a_t^{i\ast},X_t\bigr)
=\frac{\exp\bigl(\mathbb{1}\{a=a_t^{i\ast}\}/T_t\bigr)}{\sum_{a'\in\mathcal{A}^i}\exp\bigl(\mathbb{1}\{a'=a_t^{i\ast}\}/T_t\bigr)},
\label{eq:mdg}
\end{equation}
where $T_t>0$ is the temperature of the system measuring the implementation friction of the episode, not the agent's uncertainty about its own type. To capture strategic learning and institutional maturation jointly, the temperature is defined as
\begin{equation}
T_t=T_0\max\Bigl\{\frac{H(\mu_t)}{H(\mu_0)}e^{-\eta_{\mathrm{cal}}t},\,\underline{c}\Bigr\},
\label{eq:temperature}
\end{equation}
with $T_0>0$ as the initial temperature, $\eta_{\mathrm{cal}}>0$ the calendar decay rate, $H(\mu_t)=-\sum_\theta\mu_t(\theta)\ln\mu_t(\theta)$ the entropy of \citet{Shannon1948}, and $\underline{c}\in(0,1]$ the floor of structural noise. When the posterior concentrates $H(\mu_t)\to 0$, then $T_t\to 0$ and execution aligns with the rational plan; the floor $\underline{c}>0$ guarantees $T_t\ge T_0\,\underline{c}>0$, preserving the full support of~\eqref{eq:mdg} even in this limit. This condition is consistent with the absolute continuity or ``grain of truth'' requirement of \citet{KalaiLehrer1993}, according to which rational learning must not assign zero probability to events that can occur.

The physical outcome $m_t$ is an observable categorical variable on the support $\mathcal{M}_{\mathrm{out}}$, where $\mathrm{cont}$ represents the continuation of captivity and the four causes of closure are: $j=1$ payment of the ransom by the family, $j=2$ death of the victim in captivity, $j=3$ tactical rescue by the state, and $j=4$ an exogenous accounting closure channel with constant baseline intensity $\lambda_4>0$. Conditional on the captor's type and the material state $\mathcal{C}_t(\theta_K)$, its distribution is obtained from the effective intensities $\tilde{\lambda}_j(t\mid\theta_K)$. Denote by
\[
...
The modeling of risk intensities starts from distinguishing between the instantaneous intensity and its daily probabilistic projection, in line with the proportional hazards tradition of \citet{Cox1972}. The cause-specific intensity $\lambda_j(t)$ measures the marginal propensity to close due to cause $j$ as a rate of occurrence per unit of time; thus, it can take any positive value and is not bounded by one. The discrete hazard $h_j(t)\in[0,1]$, on the other hand, is the probability that event $j$ materializes during interval $t$ conditional on the system having survived to the beginning of the period. Both objects depend on the vector of executed actions $\tilde{A}_t$ generated by the MDG process and not on the latent strategic intentions; the state's continuous instruments $(\alpha_t^\ast,\gamma_t^\ast)$ modify them through the material state $\mathcal{C}_t(\theta_K)$.
...
\label{eq:lambda-j}
\end{equation}
where $\lambda_{j0}(t)\ge 0$ is the baseline risk of cause $j$, $x_t$ is a design vector encoding the state and action space, and $\beta_j$ are the technology coefficients. The effective intensities incorporate a logistical maturation filter $M(t)$ that penalizes efficiency in the early stages:
\begin{equation}
...
\end{align}
so that $h_j(t)=q(t)\xi_j(t)$ is the marginal hazard of cause $j$ and $\sum_j h_j(t)=q(t)\leq 1$ by construction. The probability $\xi_j(t)$ acts as the market share of each risk and orthogonalizes two dimensions of the process: the governance of duration (the when), determined by the aggregate density of the intensities, and the governance of the nature of the closure (the how), governed by the relative weights. It is through $\xi_j(t)$ that Bayesian learning exerts its strategic role: as $\mu_t$ concentrates on the true type, the probability mass is redistributed among exit causes, shifting the likelihood toward the outcomes that are more probable under that type, without altering the temporal scale of captivity. The probability of detecting collusion $p_{\mathrm{det},t}$ is specified through a logistic response function conditioned on the state's instruments:
\begin{equation}
p_{\mathrm{det},t}=\Lambda(\eta_0+\eta_1\alpha_t^\ast+\eta_2\gamma_t^\ast),\qquad\Lambda(u)=\frac{1}{1+\exp(-u)},
\label{eq:detection}
\end{equation}
where $\eta_0$ is the baseline level of detection, and $\eta_1>0$ and $\eta_2>0$ capture the marginal effect of the financial block and operational pressure on the probability of detecting collusion. The logistic function $\Lambda(\cdot)$ guarantees that $p_{\mathrm{det},t}\in(0,1)$ for any combination of instruments and that the detection probability is monotonically increasing in $(\alpha_t^\ast,\gamma_t^\ast)$: greater financial interdiction or higher operational pressure raises the probability of observing $d_t=1$, reducing the space for clandestine action by the family and the captor.
...
\label{eq:LH-out}
\end{equation}
Both regimes are unified in the compact expression
\begin{equation}
\Prob(m_t\mid\theta_K,\mathcal{C}_t)=p_{\mathrm{Cont},t\mid\theta_K}^{\,\mathbb{1}\{m_t=\mathrm{cont}\}}\prod_{j=1}^{4}\bar h_j(t\mid\theta_K)^{\,\mathbb{1}\{m_t=j\}},
\label{eq:LH-compacta}
\end{equation}
which corresponds to a standard categorical multinomial where each day constitutes a trial with five mutually exclusive and exhaustive cells.
...
\begin{equation}
\Prob(d_t\mid\alpha_t^\ast,\gamma_t^\ast)=p_{\mathrm{det},t}^{\,d_t}(1-p_{\mathrm{det},t})^{1-d_t}.
...
\begin{equation}
\mathcal{L}_{F,t}(\theta_K)=\underbrace{\mathcal{L}_{I,t}(\theta_K)}_{\text{executed actions}}\cdot\underbrace{\Prob(m_t\mid\theta_K,\mathcal{C}_t)}_{\text{competing risks}}\cdot\underbrace{\Prob(d_t\mid\alpha_t^\ast,\gamma_t^\ast)}_{\text{detection}}.
\label{eq:LF-joint}
\end{equation}
...
For the convergence of Lemma~\ref{lem:asymptotic-beliefs} to concentrate on the true type rather than a diffuse limit, it is necessary to restrict the analysis to the event in which captivity generates an infinite sequence of informative public signals.
...
We say that the model is asymptotically identifiable in $\mathcal{T}$ if $\ProbC{\theta_K^\ast}(\mathcal{T})>0$ and, for all $\theta\in\ThetaK$ with $\theta\neq\theta_K^\ast$, at least one of the following conditions is satisfied:
\begin{enumerate}
...
Consider a discrete-time Bayesian game with horizon $\mathbb{T}=\{0,1,\ldots,\tau\}$, players $\mathcal{J}=\{S,K,F\}$, and captor's type $\theta_K\in\Theta_K$ according to a common prior $\mu_0\in\Delta(\Theta_K)$ with $\mu_0(\theta)>0$ for all $\theta\in\Theta_K$. Following \textcite{FudenbergTirole1991} and \textcite{KrepsWilson1982}, a dynamic mechanism is described by the following formal objects:
...
\begin{itemize}
  ...
  \item \emph{Belief system:} $\mu=\{\mu_t\}_{t\ge 0}$, with $\mu_t\in\Delta(\Theta_K)$, summarizing the uncertainty regarding $\theta_K$ after what has been publicly observed up to the opening of $t$.
...
\begin{enumerate}
  ...
  \item[\textup{(b)}] On-path Bayesian consistency. In every node reached with positive probability under $\ProbE^s$, beliefs coincide with the Bayesian inference from $\mu_0$ and the observed public signals. For all $\theta\in\Theta_K$ and all $t\ge 1$,
  \begin{equation}
...
Let $(s^\ast,\mu^\ast)$ be a PBE in the sense of Definition~\ref{def:pbe-mechanism}. We say that the mechanism is implementable Nash-Bayes-$IC^K$,$IR^K$,$IR^F$ if the equilibrium profile additionally satisfies the following three feasibility conditions.

\smallskip
\noindent Bayesian incentive compatibility of the captor \textup{(}$IC^K$\textup{)}. For all $t\in\mathbb{T}$ and all rival mimicry $\theta_j\in\Theta_K$,
\[
...
\begin{list}{}{\setlength{\labelwidth}{2.6em}\setlength{\labelsep}{0.7em}\setlength{\leftmargin}{3.3em}\setlength{\itemindent}{0pt}\setlength{\listparindent}{0pt}\setlength{\parsep}{0pt}\setlength{\itemsep}{0.35\baselineskip}}
  \item[\textup{(i)}] $|\Theta_K|<\infty$, with strictly positive prior $\mu_0(\theta)>0$ for all $\theta\in\Theta_K$.
  ...
  \item[\textup{(iii)}] The implementation law $\ProbI{i}$ of the MDG process assigns strictly positive probability to every action $a\in\mathcal{A}^i$ for every plan $a_t^{i\ast}$ and state $X_t$ \textup{(}full support, \eqref{eq:mdg}\textup{)}.
  \item[\textup{(iv)}] The instrument sets are $\mathcal{A}^{\alpha}=[0,1]$ and $\mathcal{A}^{\gamma}=[0,1]$, compact and convex; the functions $L_t$, $G$, and $\mathcal{U}_t^i$ are measurable, uniformly bounded in $\mathbb{T}$, and continuous in the continuous decision variables.
  \item[\textup{(v)}] Robust feasibility. For each $t$ and each $\mu_t\in\Delta(\Theta_K)$, the feasible set $\Gamma_t(\mu_t)$ defined in~\eqref{eq:gamma-factible} is non-empty and compact. For each discrete branch $a_t^S$, its continuous section
  \[
  \Gamma_t^{a^S}(\mu_t):=\{(\alpha_t,\gamma_t):(a_t^S,\alpha_t,\gamma_t)\in\Gamma_t(\mu_t)\}
  \]
  is convex, and the correspondence $\mu_t\mapsto\Gamma_t(\mu_t)$ is continuous \textup{(}upper and lower hemicontinuous\textup{)} and compact-valued in $\Delta(\Theta_K)$. Non-emptiness is a primitive condition of simultaneous compatibility among $IC^K$, $IR^K$, and $IR^F$.
  ...
  \item[\textup{(vii)}] Quasiconvexity of the restricted objective. For each $t$, each $\mu_t\in\Delta(\Theta_K)$, and each discrete branch $a_t^S\in\mathcal{A}^{S,\mathrm{disc}}$, the functional
  \[
...
\]
The elasticity of $\gamma_t^\ast$ with respect to $\mu_t(\theta_{high})$ depends on the differential ratio of marginal losses between types, which in turn is proportional to
\[
...
\end{equation}
If $\kappa_h(\theta_K, t) > 0$ for all $t$, the expected duration of captivity $\mathbb{E}[\tau \mid \theta_K]$ is strictly decreasing in $\gamma_t^\ast$. If $\kappa_h(\theta_K, t) < 0$ for all $t$, it is strictly increasing. The model thus generates the testable differential prediction:
\begin{equation}
...
This section calibrates the dynamic mechanism of Section~\ref{sec:model} in two simulated episodes of finite horizon. The exercise combines macro and microeconometric priors and diagnostics to study four objects: the evolution of beliefs $\mu_\tau(\theta)$, the signals observed by the state, the response of the instruments $(\alpha_t,\gamma_t)$, and compliance with the IR and IC constraints. Scenario~I corresponds to ELN, with $\theta_K^\ast=\mathrm{ELN}$, lax state, low-wealth family, public victim, and Caribbean location; Scenario~II corresponds to PAR, with $\theta_K^\ast=\mathrm{PAR}$, lax state, high-wealth family, private victim, and East--Jungle location. Both are simulated up to $\tau=150$, and optimal policy is computed from the state's minimization of social cost. In the order $(\mathrm{DC},\mathrm{PAR},\mathrm{ELN},\mathrm{FARC})$, the initial priors are $\mu_0^{\mathrm{ELN}}=(0.25,0.35,0.05,0.35)$ and $\mu_0^{\mathrm{PAR}}=(0.55,0.05,0.35,0.05)$. In the figures, the gray bar marks the first period in which the mechanism draw would terminate the kidnapping; the subsequent trajectory is retained to distinguish the absorption moment from the learning and identification dynamics.

The posterior of the true type allows evaluating the capability of the mechanism to separate types from the sequence of signals. In the ELN scenario, reported in Figure~\ref{fig:cal-eln-par-mu}, posterior mass shifts gradually toward ELN until reaching $\mu_{150}(\mathrm{ELN})\approx 1{,}000$; this implies that the mechanism accumulates identification even in an environment with lower family wealth and a public victim. In PAR, the posterior of the true type starts from $\mu_1(\mathrm{PAR})=0{,}050$ and reaches $\mu_{150}(\mathrm{PAR})\approx 1{,}000$; the trajectory is not monotone in the initial phase, because the event combines early signals compatible with other types and then shifts mass toward PAR. The gray bars mark the first period in which the outcome $m_\tau$ differs from continuation ($\tau=16$ in ELN and $\tau=5$ in PAR), but the simulation continues to show that absorption and the speed of learning are distinct objects. Taken together, the figure supports the intuition of Theorem~\ref{thm:posterior-consistency}: posterior mass concentrates or shifts toward the true type when signals accumulate separation, although the speed of convergence depends on the institutional, territorial, and case-visibility environment.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{figuras_calibracion_journal/ELN_01_mu.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{figuras_calibracion_journal/PAR_2_01_mu.png}
...
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) corresponds to Scenario~I: $\theta_K^\ast=\mathrm{ELN}$, lax $\theta_S$, low-wealth $\theta_F$, public $\theta_V$, and Caribbean $z$. Panel (b) corresponds to Scenario~II: $\theta_K^\ast=\mathrm{PAR}$, lax $\theta_S$, high-wealth $\theta_F$, private $\theta_V$, and East--Jungle $z$. The gray band marks $\tau^{\mathrm{hyp}}$, the first period in which the mechanism outcome differs from continuation.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{figuras_calibracion_journal/ELN_03_gamma.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{figuras_calibracion_journal/PAR_2_03_gamma.png}
  \caption{Scenario II: PAR}
\end{subfigure}
\caption{Operational pressure $\gamma_t^\ast$ and benchmark $\gamma^R$ against $\tau$.}
\label{fig:cal-eln-par-gamma}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) shows the operational pressure trajectory in Scenario~I (ELN). Panel (b) shows the equivalent trajectory in Scenario~II (PAR). The dashed line corresponds to the Rescue benchmark $\gamma^R$. The gray band marks $\tau^{\mathrm{hyp}}$.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{figuras_calibracion_journal/ELN_02_alpha.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{figuras_calibracion_journal/PAR_2_02_alpha.png}
  \caption{Scenario II: PAR}
\end{subfigure}
\caption{Financial interdiction $\alpha_t^\ast$ and benchmark $\alpha^R$ against $\tau$.}
\label{fig:cal-eln-par-alpha}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) presents the optimal financial interdiction in Scenario~I (ELN). Panel (b) presents the same instrument in Scenario~II (PAR). The dashed line corresponds to the Rescue benchmark $\alpha^R$. The gray band marks $\tau^{\mathrm{hyp}}$.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calpathheight}{figuras_calibracion_journal/ELN_08_gamma_iota.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calpathheight}{figuras_calibracion_journal/PAR_2_08_gamma_iota.png}
  \caption{Scenario II: PAR}
\end{subfigure}
\caption{Trajectory $(\iota_t,\gamma_t^\ast)$ with arrows indicating temporal advance.}
\label{fig:cal-eln-par-iota-gamma}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) corresponds to Scenario~I (ELN) and panel (b) to Scenario~II (PAR). Each trajectory shows 20 arrows traced over homogeneous intervals of $\tau$; the circle identifies the end point of the simulation, $\tau=150$.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calpathheight}{figuras_calibracion_journal/ELN_09_alpha_iota.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calpathheight}{figuras_calibracion_journal/PAR_2_09_alpha_iota.png}
  \caption{Scenario II: PAR}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calplotheight}{figuras_calibracion_journal/ELN_11_IRIC.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calplotheight}{figuras_calibracion_journal/PAR_2_11_IRIC.png}
  \caption{Scenario II: PAR}
\end{subfigure}
...
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) summarizes compliance with the captor's incentive compatibility, the captor's participation, and the family's participation in Scenario~I (ELN). Panel (b) presents the same diagnostic for Scenario~II (PAR). Percentages are computed over the simulated cycles of each scenario.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calfigheight}{figuras_calibracion_journal/ELN_07_DH.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calfigheight}{figuras_calibracion_journal/PAR_2_07_DH.png}
  \caption{Scenario II: PAR}
\end{subfigure}
\caption{Expected informational gain $\Delta H_t$ against $\tau$.}
\label{fig:cal-eln-par-dh}
\begin{minipage}{\linewidth}
\footnotesize \textit{Note:} Panel (a) corresponds to Scenario~I (ELN) and panel (b) to Scenario~II (PAR). The series reports the expected entropy reduction induced by the state's optimal policy in each simulated cycle.
\end{minipage}
\end{figure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calplotheight}{figuras_calibracion_journal/ELN_12_kh_true.png}
  \caption{Scenario I: ELN}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calplotheight}{figuras_calibracion_journal/PAR_2_12_kh_true.png}
  \caption{Scenario II: PAR}
\end{subfigure}

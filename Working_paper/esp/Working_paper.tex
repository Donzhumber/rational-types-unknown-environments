...
% Cuerpo 12pt, notas al pie a 9/11 (ligeramente por debajo del \footnotesize por defecto de 10pt), uso habitual en textos académicos. (etoolbox lo carga biblatex.)
\makeatletter
\AtBeginDocument{%
...
\subsection*{\Large\bfseries Introducción}
...
\subsection*{\Large\bfseries Tendencias largas en secuestro para rescate}
...
Para la estimación del modelo VEC, el análisis considera secuestro para rescate ($K^{E}$), secuestro no extorsionado ($K^{NE}$), y la tasa de interés del banco comercial ($r$). Los datos son mensuales y abarcan el período comprendido entre enero de 1964 y diciembre de 2025. Con el apoyo de herramientas de optimización basadas en inteligencia artificial, se evaluó la robustez de un conjunto de especificaciones VEC utilizando longitudes de lag que van de 1 a 50 períodos y un rango de cointegración igual a uno. Los resultados indican que la especificación óptima corresponde a un modelo VEC con 36 vueltas, ya que exhibe el mejor rendimiento en términos de la Criterio de Información de Akaike (AIC), la ausencia de autocorrelación residual y heteroskedasticidad, un error cuadrado de raíz inferior (RMSE), y ninguna evidencia de efectos de ARCH o GARCH. Los coeficientes de los vector cointegrante$\boldsymbol{\beta}$) asociado con esta especificación se reportan en la Ecuación~\ref{eq_1}.\\
...
\begin{center}
    ...
    \begin{minipage}{0.50\linewidth} 
        \begin{equation}
        \label{eq_2}
            K^{E}_t = 1.95\,K^{NE}_t + 1.32\,r_t + \varepsilon_t
        \end{equation}
    \end{minipage}
\end{center}
...
\begin{align}
    ...
    \label{eq_3} \\
    \Delta K^{NE}_t &= \underset{(p = 0.10)}{0.036} \, \tilde{\varepsilon}_{t-1} + \sum_{j=1}^{35} \Gamma_{2,j} \Delta \mathbf{X}_{t-j} + \varepsilon_{2,t} 
    \label{eq_4} \\
    \Delta r_t &= \underset{(p = 0.09)}{-0.01} \, \tilde{\varepsilon}_{t-1} + \sum_{j=1}^{35} \Gamma_{3,j} \Delta \mathbf{X}_{t-j} + \varepsilon_{3,t} 
    \label{eq_5}
\end{align}
...
\begin{aligned}
    ...
    && \hat{\sigma}_\eta \approx 7.12 \; \underset{[p<0.001]}{(0.058)} \\[10pt]
...
\\
Vamos. $\{y_t\}$ denota una serie de tiempo integrada del orden uno, $I(1)$. Asumir la existencia de dos descomposiciones válidas definidas como:
\begin{equation*}
    y_t = P_t^{(1)} + u_t^{(1)} = P_t^{(2)} + u_t^{(2)},
\end{equation*}
Donde $P_t^{(1)}$ y $P_t^{(2)}$ son procesos integrados del orden uno, y $u_t^{(1)}, u_t^{(2)}$ son estacionarios $I(0)$ procesos. De ahí que:
\begin{equation*}
    P_t^{(1)} - P_t^{(2)} \sim I(0).
\end{equation*}
En consecuencia, todas las tendencias estocásticas válidas derivadas de la misma serie observada equivalen a un componente estacionario.
\end{corollary}
...
\subsection*{\Large\bfseries Análisis microeconómico del secuestro para rescate}

\subsection*{\normalfont\itshape Manejo de datos y construcción de muestras}
...
Tipología extorsionada confirmada & 11,540 & 29.3\% & 70.7\% \\
Resultado válido ($Y$) Información & 4,562 & 11.6\% & 88.4\% \\
Covariados completos (pre-estimación) & 1,133 & 2.9\% & 97.1\% \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.5cm} p{3.5cm} p{5.5cm} p{2.5cm}@{}}
        ...
        & $z=-5.68$ \newline $p<0.001$
        & \textbf{Heterogeneidad estructural:} grupos delictivos comunes muestran una tasa de letalidad significativamente mayor (7.9\%) que las FARC (0.2\%), revelando diferencias en los costos asociados con el fracaso criminal.
        & \textbf{Significado} \newline ($p<0.01$) \\
        \addlinespace
        \textbf{Attrición / Prueba de Logit descomunal}
        & ratios de probabilidades significativas \newline ($p<0.05$)
        & \textbf{Reporting bias:} La probabilidad de falta de información sobre los resultados es rara vez y aumenta en las regiones periféricas y los contextos de menor capacidad estatal.
        & \textbf{Limitación} \newline (reportado) \\
        \bottomrule
    \end{tabular}
    ...
    \begin{minipage}{0.98\linewidth}
        \footnotesize
        \justifying
        \textit{Nota:} La prueba IIA evalúa la suposición de Independencia de Alternativas Irrelevant comparando los coeficientes del modelo completo con los de un modelo restringido utilizando el enfoque SUEST. La prueba de letalidad corresponde a a diferencia-en-proporción test across armed groups. La prueba de atrición se basa en un modelo de logit que estima la probabilidad de que el resultado del secuestro no se conserva. \\ \scriptsize\textit{Fuente:} Los cálculos de los autores usando Stata 19.
    \end{minipage}
\end{table}


\subsection*{\normalfont\itshape Determinantes estratégicos y dinámicas temporales de secuestro}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.95\textwidth}
        \footnotesize
        \textit{Notas:} Panel (a) muestra la probabilidad acumulativa de que un rescate estatal haya ocurrido por cada punto en el tiempo. Grupo b). El panel (b) muestra la probabilidad promedio de muerte en cautiverio por grupo armado, calculada como la media de un indicador binario de indicador de muerte dentro de cada categoría. Los valores reportados son proporciones descriptivas y no se derivan de un modelo de duración.
        \\ \textit{Fuente:} Los cálculos de los autores usando Stata 19, basados en datos del \textcite{cnmh_datos}.
    \end{minipage}
...
\subsection*{\normalfont\itshape  Riesgo de secuestro a nivel poblacional: evidencia de una encuesta nacional}
...
\subsection*{\Large\bfseries Diseño de mecanismos para la disuasión y mitigación del secuestro extorsivo}
...
% =========================
\section{Conclusión}
Resumir las conclusiones, limitaciones e implicaciones.
...
\title{Identificación de tipos racionales en entornos desconocidos:\\
\large El caso del secuestro en Colombia II}
\author{Humberto Bernal\thanks{Universidad Mayor de Cundinamarca. Ph.D.\ en Economía, Universidad de los Andes, Colombia. \href{mailto:hbeernalc@universidadmayor.edu.co}{hbeernalc@universidadmayor.edu.co}; \href{mailto:hbernal@uniandes.edu.co}{hbernal@uniandes.edu.co}}}
\date{}
\maketitle

\begin{abstract}
Desarrollamos un mecanismo dinámico de disuasión del secuestro extorsivo en tiempo discreto bajo información incompleta. Tres actores---Estado, captor y familia---interactúan a lo largo de un cautiverio modelado como un proceso de riesgos competitivos proporcionales en covariables. El tipo del captor $\theta_K$ es información privada; el Estado actualiza sus creencias mediante aprendizaje bayesiano sobre una historia pública de desenlaces y señales observables. La implementación descansa en el proceso MDG (Mano de Dios-Guadalupe), que introduce una capa estocástica entre la intención estratégica y la acción ejecutada, garantizando la consistencia secuencial del Equilibrio Bayesiano Perfecto fuera de la senda de equilibrio. El resultado central---el \textbf{Teorema de Identificación de Tipos bajo Entornos Desconocidos}---establece que si la heterogeneidad estructural en los coeficientes de riesgo $\beta_{K,j}$ genera separación uniforme en variación total entre las leyes diarias de observación, el criterio de Kakutani implica singularidad mutua de las leyes de trayectoria y las creencias posteriores convergen al tipo verdadero casi seguramente. Las restricciones de compatibilidad de incentivos (IC) y participación (IR) anclan la verosimilitud predictiva al modelo estructural, convirtiendo el comportamiento observable en evidencia auditable sobre la identidad del adversario.
\end{abstract}
\vspace{0.5\baselineskip}
\noindent{\small\textbf{Palabras clave.} mecanismos dinámicos; información incompleta; aprendizaje bayesiano; equilibrio bayesiano perfecto; secuestro.}\par\nobreak
...
\begin{enumerate}
  ...
  \item[\textup{(ii)}] \textbf{Convergencia institucional.} El ruido de implementación puede ser no estacionario. En un régimen de \textbf{horizonte largo}, se puede imponer que la probabilidad de error se desvanezca conforme el sistema acumula información.
  \begin{equation}
  \label{eq:mano-dios-evanescente}
  \lim_{t\to\infty}{\ProbE}\bigl(\tilde{a}_t^i=a_t^{i\ast}\bigm|\mathcal{F}_t^{\mathrm{pub}}\bigr)=1, \quad \forall\,i.
  \end{equation}
  Esta condición se satisface cuando $H(\mu_t)\to 0$ \textup{(}posterior concentrada\textup{)}, pues entonces $T_t\to 0$ vía el factor de entropía en~\eqref{eq:temperatura-piso} ---definida a continuación---, con independencia del piso $\underline{c}>0$, el cual solo limita la componente $e^{-\eta_{\mathrm{cal}}t}$. No debe interpretarse como convergencia por mero decaimiento calendario.
\end{enumerate}
\end{definition}
...
T_t = T_0 \left[\frac{H(\mu_t)}{H(\mu_0)}\right]\max\{e^{-\eta_{\mathrm{cal}}t},\,\underline{c}\},
\end{equation}
\noindent donde $T_0>0$ es la temperatura inicial, $\eta_{\mathrm{cal}}>0$ la tasa de decaimiento calendario, $H(\mu_t)=-\sum_{\theta}\mu_t(\theta)\ln\mu_t(\theta)$ la entropía de Shannon \textcite{Shannon1948} y $\underline{c}\in(0,1]$ el piso de ruido estructural. Cuando la posterior se concentra, $T_t\to 0$ y la ejecución se alinea con el plan racional; el piso $\underline{c}>0$ garantiza $T_t\ge T_0\,\underline{c}>0$, preservando el soporte completo de~\eqref{eq:logit-hybrid} incluso si $H(\mu_t)\to 0$. $T_t$ mide la fricción de implementación del episodio, no la incertidumbre del agente sobre su propio tipo.
...
p_{\mathrm{det},t}
=
\Lambda(\eta_0 + \eta_1\alpha_t^\ast + \eta_2\gamma_t^\ast),
\qquad
...
= \exp\!\Bigl(-\sum_{j=1}^{3}\tilde{\lambda}_j(s\mid\theta_K)\,\Delta t\Bigr),
\quad
\tilde{\lambda}_j(s\mid\theta_K)
= \lambda_{j0}(s)\exp\!\bigl(\beta_{K,j}(\theta_K)\cdot z+\zeta_{\gamma,j}\,\gamma_s^\ast\bigr),
\]
de modo que, diferenciando respecto a $\gamma_t^\ast$,
\begin{equation}
...
\ProbE\bigl(d_t \,\big|\, \alpha_t^\ast,\gamma_t^\ast\bigr)=p_{\mathrm{det},t}^{\,d_t}(1-p_{\mathrm{det},t})^{\,1-d_t}, \qquad p_{\mathrm{det},t}=\ProbE\bigl(d_t=1\mid \alpha_t^\ast,\gamma_t^\ast\bigr),
\end{equation}
...
\cdot
\underbrace{\ProbE(d_t \mid \alpha_t^\ast,\gamma_t^\ast)}_{\text{detección}},
\end{equation}
...
\prod_{j=1}^{4} h_j(t\mid\theta_K)^{\,\mathbf{1}\{m_t=j\}}
\right]
p_{\mathrm{det},t}^{\,d_t}(1-p_{\mathrm{det},t})^{\,1-d_t},
\end{equation}
donde $\mathcal{L}_{I,t}$ está definida en~\eqref{eq:LI-atilde}, $\tilde{\lambda}_j(t\mid\theta_K;\alpha_t^\ast,\gamma_t^\ast)$ determina $p_{\mathrm{Cont},t\mid\theta_K}$ y $h_j(t\mid\theta_K)$, y $p_{\mathrm{det},t}=\Lambda(\eta_0+\eta_1\alpha_t^\ast+\eta_2\gamma_t^\ast)$. Al incorporar la comunicación, la \textbf{regla de Bayes unificada} del modelo es
\begin{equation}
\label{eq:bayes-unif}
\mu_{t+1}(\theta_K)
=
...
\end{equation}
con la convención $0\log0=0$. Para cada desenlace físico $m\in\{\mathrm{Cont},\mathrm{rel},\mathrm{res},\mathrm{pay},\mathrm{kill}\}$, el Estado calcula la posterior contrafactual que resultaría de observar $m$ bajo la política candidata:
\begin{equation}
\label{eq:posterior-counterfactual-entropy}
\mu_{t+1}^{m}(\theta)
=
\frac{
\mu_t(\theta)\ProbE(m_t=m\mid\theta,\alpha_t,\gamma_t,\mathcal C_t)
}{
\sum_{\theta'\in\Theta_K}
\mu_t(\theta')\ProbE(m_t=m\mid\theta',\alpha_t,\gamma_t,\mathcal C_t)
}.
\end{equation}
La ganancia esperada de información es la reducción esperada de entropía:
\begin{equation}
...
-
\sum_m
\ProbE(m_t=m\mid\mu_t,\alpha_t,\gamma_t,\mathcal C_t)
H(\mu_{t+1}^{m}),
\end{equation}
donde la probabilidad predictiva $\ProbE(m_t=m\mid\mu_t,\alpha_t,\gamma_t,\mathcal C_t)$ es la mezcla bayesiana de los riesgos competitivos~\eqref{eq:xi}. El término $-\psi_H\Delta H_t$ opera como subsidio informacional: si $\psi_H=0$, el Estado es puramente miópico; si $\psi_H>0$, las políticas que compran aprendizaje reciben un descuento en la pérdida. Cuando $\mu_t$ se concentra en el tipo verdadero, $H(\mu_t)$ y $\Delta H_t$ caen, apagando endógenamente el motivo exploratorio.
...
% ============================================================
\section{Calibración y resultados cuantitativos}
\label{sec:calibracion-resultados}

Esta sección documenta una calibración computacional del mecanismo con dos tipos verdaderos contrastantes, $\theta_K^\ast=\mathrm{DC}$ y $\theta_K^\ast=\mathrm{FARC}$. El ejercicio no pretende ser una estimación estructural completa; su objetivo es verificar que las primitivas introducidas en el modelo generan trayectorias cuantitativas coherentes de aprendizaje bayesiano, elección óptima de instrumentos y valor informacional. Las series se obtienen en \textbf{modo Continuar}: el proceso sigue hasta un horizonte fijo $T=80$ aun cuando el sorteo físico diario produzca un desenlace terminal. El primer periodo en que tal desenlace habría absorbido el episodio se denota
\[
\tau^{\mathrm{hyp}}:=\inf\{t\geq0:m_t\neq\mathrm{Continuar}\}.
\]
En todas las figuras temporales, la banda gris claro marca $\tau^{\mathrm{hyp}}$. Por tanto, la parte de la trayectoria ubicada a la derecha de la banda debe leerse como dinámica contrafactual de aprendizaje y política: muestra cómo habría evolucionado el mecanismo si el episodio no se hubiera censurado por absorción.

La simulación utiliza el prior $\mu_0=(0.25,0.20,0.20,0.35)$ para $(\mathrm{DC},\mathrm{PAR},\mathrm{ELN},\mathrm{FARC})$, región metropolitana, víctima privada y semilla fija. En cada periodo se calcula una política $(\alpha_t^\ast,\gamma_t^\ast)$ por búsqueda en grilla sobre el problema del Estado, incorporando la pérdida esperada por pago, muerte, rescate y liberación, costos convexos de instrumentos y el término de ganancia informacional $\Delta H_t$. Las referencias $\alpha^R,\gamma^R$ y $\alpha^N,\gamma^N$ corresponden a los mínimos de las ramas de rescate y negociación sin el bono informacional; la política efectiva selecciona la rama de menor pérdida y luego internaliza $\Delta H_t$.

\input{figuras_calibracion/tabla_resumen_calibracion.tex}

El Cuadro~\ref{tab:calibracion-resumen} resume la corrida. En ambos casos la posterior del tipo verdadero aumenta, pero la concentración es más marcada para FARC: $\mu_T(\mathrm{FARC})=0.924$, frente a $\mu_T(\mathrm{DC})=0.728$. La diferencia es consistente con la separación de hazards del modelo: en esta parametrización, la realización asociada a FARC produce una señal terminal de rescate temprana, mientras que DC produce pago en $\tau^{\mathrm{hyp}}=25$. La precisión media $\bar\iota$ también es mayor para FARC, lo que indica que la historia pública discrimina más rápidamente el tipo organizado que el tipo financiero de menor estructura.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_mu_dc_farc.pdf}
\caption{Aprendizaje bayesiano en modo Continuar. La línea negra muestra la posterior del tipo verdadero y la banda gris marca $\tau^{\mathrm{hyp}}$.}
\label{fig:calibracion-mu}
\end{figure}

La Figura~\ref{fig:calibracion-mu} muestra el objeto central del teorema de identificación en horizonte finito. Para DC, la posterior crece de forma menos monotónica y solo alcanza una concentración moderada al final del horizonte, lo que refleja que varias realizaciones de continuidad y pago siguen siendo compatibles con otros tipos. Para FARC, la masa posterior se desplaza con mayor rapidez hacia el tipo verdadero y permanece alta después de la banda gris. La lectura relevante es temporal: antes de $\tau^{\mathrm{hyp}}$, el Estado ya acumula información; después de ese punto, la trayectoria ilustra el aprendizaje contrafactual que se perdería si el episodio quedara censurado por absorción.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_alpha_dc_farc.pdf}
\caption{Bloqueo financiero óptimo y referencias de rama. La línea principal reporta $\alpha_t^\ast$; las líneas grises muestran $\alpha^R$ y $\alpha^N$.}
\label{fig:calibracion-alpha}
\end{figure}

La Figura~\ref{fig:calibracion-alpha} muestra el componente financiero de la política. En el caso DC, $\alpha_t^\ast$ fluctúa alrededor de $\bar\alpha^\ast=0.356$ y se aproxima con frecuencia a la referencia de negociación, consistente con un tipo cuya señal terminal hipotética es pago. Para FARC, el bloqueo medio es menor, $\bar\alpha^\ast=0.310$, y la serie es más estable antes y después de $\tau^{\mathrm{hyp}}$. La comparación con $\alpha^R$ y $\alpha^N$ muestra que la regla efectiva no copia mecánicamente una rama pura: el bloqueo se desplaza con la posterior y con la contribución informacional de la señal esperada.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_gamma_dc_farc.pdf}
\caption{Presión operativa óptima y referencias de rama. La línea principal reporta $\gamma_t^\ast$; las líneas grises muestran $\gamma^R$ y $\gamma^N$.}
\label{fig:calibracion-gamma}
\end{figure}

La Figura~\ref{fig:calibracion-gamma} separa el margen operativo. Para DC, $\gamma_t^\ast$ tiene media $\bar\gamma^\ast=0.366$ y exhibe mayor movilidad, reflejando que la presión tiene un doble papel: altera hazards competitivos y preserva información cuando el tipo permanece menos identificado. Para FARC, $\bar\gamma^\ast=0.318$ y la trayectoria se estabiliza más rápido, en línea con una posterior que alcanza mayor concentración. Las referencias $\gamma^R$ y $\gamma^N$ permiten leer si la política se acerca a una lógica de rescate o de negociación sin mezclarla con el instrumento financiero.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_iota_alpha_dc_farc.pdf}
\caption{Precisión posterior y bloqueo financiero. El eje horizontal es $\iota_t=\max_\theta\mu_t(\theta)$ y el eje vertical es $\alpha_t^\ast$; el marcador abierto indica $\tau^{\mathrm{hyp}}$.}
\label{fig:calibracion-iota-alpha}
\end{figure}

La Figura~\ref{fig:calibracion-iota-alpha} reordena la trayectoria por precisión posterior, no por calendario. En DC, la nube ordenada muestra que el bloqueo se ajusta en un rango amplio mientras $\iota_t$ permanece relativamente bajo; esto es coherente con una economía de aprendizaje en la que la política conserva valor experimental. En FARC, la masa de puntos se desplaza hacia valores altos de $\iota_t$ y el bloqueo se concentra en un intervalo más estrecho. El marcador de $\tau^{\mathrm{hyp}}$ ubica el punto en que el episodio se habría absorbido dentro de esa relación política-información.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_iota_gamma_dc_farc.pdf}
\caption{Precisión posterior y presión operativa. El eje horizontal es $\iota_t$ y el eje vertical es $\gamma_t^\ast$; el marcador abierto indica $\tau^{\mathrm{hyp}}$.}
\label{fig:calibracion-iota-gamma}
\end{figure}

La Figura~\ref{fig:calibracion-iota-gamma} presenta el margen central de la Proposición~\ref{prop:optimal-gamma-beliefs}. En DC, la presión operativa se mueve más mientras la precisión posterior sigue incompleta, lo que refleja el trade-off entre forzar cierre y aprender del tipo. En FARC, la relación se estabiliza con mayor rapidez: una vez que $\iota_t$ aumenta, el valor marginal de cambios adicionales en $\gamma_t^\ast$ cae. Esta lectura complementa la figura temporal: no solo importa cuándo cambia la presión, sino con qué grado de identificación posterior ocurre ese cambio.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figuras_calibracion/fig_deltaH_dc_farc.pdf}
\caption{Ganancia informacional esperada $\Delta H_t$. La banda gris marca el punto de absorción hipotética bajo el sorteo físico diario.}
\label{fig:calibracion-deltah}
\end{figure}

Finalmente, la Figura~\ref{fig:calibracion-deltah} muestra la ganancia esperada de entropía. En ambos tipos, $\Delta H_t$ es mayor cuando las creencias conservan incertidumbre suficiente para que una señal pública reordene la posterior; luego cae al concentrarse la masa. El nivel medio es ligeramente mayor para DC, $\overline{\Delta H}=0.0043$, que para FARC, $\overline{\Delta H}=0.0039$, precisamente porque la trayectoria DC permanece menos identificada. La banda gris permite separar dos mensajes: antes de la absorción hipotética, $\Delta H_t$ mide aprendizaje empíricamente observable; después, mide el valor informacional que el investigador solo observa en el laboratorio contrafactual del mecanismo.
...
Tres elementos estructurales sostienen este resultado. Primero, el proceso MDG (Mano de Dios-Guadalupe), justificado axiomáticamente como la única ley de implementación compatible con soporte completo, dominancia y máxima entropía (Observación~\ref{rem:axiomas-logit}), garantiza que la historia pública permanezca informativa incluso fuera de la senda de equilibrio. Segundo, el Teorema de Implementabilidad (Teorema~\ref{thm:ebp-implementabilidad}) establece que existe un Equilibrio Bayesiano Perfecto que implementa la regla de elección social bajo compatibilidad de incentivos $IC^K$, racionalidad individual $IR^K$ e $IR^F$. Tercero, la sección de identificación estructural (Sección~\ref{app:general-id}) provee el fundamento measure-theoretic: la singularidad mutua de las leyes de trayectoria ---vía el criterio de Kakutani--- es el mecanismo que convierte la separación estadística local en identificación asintótica global.

Las implicaciones de política (Sección~\ref{sec:policy-implications}) son operacionales: la presión operativa óptima $\gamma_t^\ast$ debe calibrarse según las creencias posteriores del Estado y la separación estructural entre tipos, y su efecto sobre la duración del cautiverio depende de la sensibilidad neta $\kappa_h(\theta_K,t)$ de los hazards competitivos. Una agenda de investigación futura incluye la extensión a tipos continuos $\Theta_K\subseteq\mathbb{R}$, la incorporación de comunicación estratégica endógena entre familia y Estado, y la estimación estructural del modelo con datos panel de episodios de secuestro en Colombia.
